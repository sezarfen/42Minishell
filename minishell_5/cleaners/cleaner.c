...
char	*find_in_env(char *str, int *i, t_env *env) // $ işaretinin başından geliyor $USER gibi
{
	...
	n = 1; // "$USER"  // $USER$USER problemli
	while (str[n] != ' ' && str[n] != '"' && str[n] != '\'' && str[n] && str[n] != '$') // "$US'" gibi bir case ' de durduruyor ama zaten US' diye bir değişken yok, özellikle bir şey denemek isteyenler için handle edilebilir
		n++;
	*i += n;
	j = 0;
	while (env->key[j])
...
{	// [ echo "hello"'world' "$USER"'$USER' ] -> [ echo helloworld fdundar$USER ]
	char	*clean;
	int		len;
	int		i;
	int		k;
	...
	len = ft_strlen(str); // kelimeler ayrı ayrı geliyor (echo) ("hello"'world')
	clean = ft_strdup("");
	while (i <= len)
	{
		if (str[i] == '"') // asd"hello"123'jkl'
		{
			...
			{
				if (str[i] == '$')
				{// buranın öncesinde bir string olabilir // "hello$USER"
					clean = ft_strjoin_df(clean, ft_substr(str, k, i - k));
					clean = ft_strjoin_df(clean, find_in_env(str + i, &i, env));
					k = i; // önemli bir nokta olabilir
				}
				if (str[i] != '"' && str[i] != '$')
					i++;
			}
